...
#
# Usage:
#   curl -fsSL https://raw.githubusercontent.com/koboldul/hermes-agent/main/scripts/install.sh | bash
#
# Or with options:
...
# Configuration
REPO_URL_SSH="git@github.com:koboldul/hermes-agent.git"
REPO_URL_HTTPS="https://github.com/koboldul/hermes-agent.git"
...
check_node() {
    ...
    if command -v node &> /dev/null && ! command -v npm &> /dev/null; then
        ...
        log_warn "Node.js on PATH is untrusted or unsupported (Hermes requires Node 22.22+, 24, or 26+) — installing Hermes-managed Node $NODE_VERSION..."
