...
$script:HasNode = $null
$NodeVersion = '22'
# This harness verifies the compatibility fallback itself. Production keeps
# that unverified download disabled unless the operator passes the matching
# installer switch.
    ...
    switch ($Name) {
        ...
        'npm.cmd' {
            if ($script:FakeNpmAvailable) {
                return [pscustomobject]@{ Source = 'npm.cmd' }
            }
            return $null
        }
        'npm' { return $null }
