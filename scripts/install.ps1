...
#
# Usage:
#   iex (irm https://raw.githubusercontent.com/koboldul/hermes-agent/main/scripts/install.ps1)
#
...
$RepoUrlSsh = "git@github.com:koboldul/hermes-agent.git"
$RepoUrlHttps = "https://github.com/koboldul/hermes-agent.git"
$RepoArchiveBase = "https://github.com/koboldul/hermes-agent/archive"
$PythonVersion = "3.11"
...
function Test-SystemNodeReady {
    ...
    $pathNode = if ($nodeCommand) { $nodeCommand.Source } else { $null }
    $pathNpm = if ($npmCommand) { $npmCommand.Source } else { $null }

    # A PATH alias into a Hermes-managed tree is executable only after the
    # single-binary and whole-tree provenance markers both verify.
    ...
    if (-not $acceptedNode -or -not $acceptedNpm) { return $false }
    ...
    $npmVersion = $null
    try {
        $npmVersion = (& $acceptedNpm --version 2>$null | Select-Object -First 1)
    } catch { }
...
# are unchanged. The internal bridge also supports hosts whose invocation
# metadata does not preserve the dot-source marker.
if ($env:_HERMES_PS_DOTSOURCE_ONLY -eq "1" -or $MyInvocation.InvocationName -eq ".") {
    return
}
