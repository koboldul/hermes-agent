...
def _expected_temp_path(path) -> str:
    value = str(path)
...
def test_temp_dir_override_honored(tmp_path):
    ...
    assert env.get_temp_dir() == _expected_temp_path(target)


@pytest.mark.windows_only
def test_windows_temp_dir_override_is_shell_safe(tmp_path):
    target = str(tmp_path)
    env = _make_local_env({"TERMINAL_TEMP_DIR": target})
    assert env.get_temp_dir() == target.replace("\\", "/")
...
def test_temp_dir_empty_falls_through(tmp_path, monkeypatch):
    ...
    assert env.get_temp_dir() == _expected_temp_path(tmp_path)
...
def test_default_is_hermes_cache_not_tmp(tmp_path, monkeypatch):
    ...
    assert result == _expected_temp_path(tmp_path / ".hermes" / "cache" / "terminal")
