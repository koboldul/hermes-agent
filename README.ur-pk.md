...
> **پیشگی شرط — پہلے `uv` انسٹال کریں۔** ہرمیس اپنا Python ماحول [`uv`](https://docs.astral.sh/uv/) (Astral کا پیکج مینیجر) سے بناتا ہے۔ سپلائی چین کی حفاظت کے لیے انسٹالر `uv` خود بخود ڈاؤنلوڈ **نہیں** کرتا؛ یہ آپریٹر کا انسٹال کردہ `uv` استعمال کرتا ہے اور نہ ملنے پر ہدایات کے ساتھ رک جاتا ہے۔ پہلے اسے انسٹال کریں:
>
> - **Linux / macOS / WSL2:** `pipx install uv` — یا `brew install uv` — یا [uv کی آفیشل گائیڈ](https://docs.astral.sh/uv/getting-started/installation/)۔
> - **Windows (PowerShell):** `winget install astral-sh.uv`
>
> `uv --version` سے تصدیق کریں۔ Node.js اور Git کے ساتھ بھی یہی سلوک ہے — اگر انسٹالر انہیں غائب بتائے تو اپنے پیکج مینیجر سے انسٹال کریں۔ (**Android/Termux مستثنیٰ ہے** — یہ Python کے stdlib `venv` + `pip` استعمال کرتا ہے، `uv` کی ضرورت نہیں۔)
...
`uv` انسٹال ہونے کے بعد (اوپر دی گئی پیشگی شرط دیکھیں)، انسٹالر باقی چیزیں سنبھالتا ہے: Python 3.11، ripgrep، ffmpeg، **اور ایک پورٹ ایبل (portable) گٹ بیش (Git Bash)** (یعنی MinGit، جو `%LOCALAPPDATA%\hermes\git` میں ان پیک ہوتا ہے — اس کے لیے ایڈمن کی اجازت درکار نہیں، اور یہ سسٹم کے کسی بھی گٹ انسٹال سے بالکل الگ ہے)۔ ہرمیس اس بنڈل شدہ گٹ بیش کو شیل کمانڈز چلانے کے لیے استعمال کرتا ہے۔ اگر `uv`، Node.js، یا Git غائب ہوں تو انسٹالر رک کر انہیں انسٹال کرنے کی درست کمانڈ بتاتا ہے (`winget install astral-sh.uv`) بجائے اس کے کہ غیر تصدیق شدہ بائنریز ڈاؤنلوڈ کرے۔
