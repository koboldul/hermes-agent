...
class LocalEnvironment(BaseEnvironment):
    ...
    def get_temp_dir(self) -> str:
        ...
        override the temp root explicitly (for example via terminal.temp_dir or
        a custom TMPDIR), then fall back to the host process environment.
        ...
        **Windows:** hardcoded ``/tmp`` is wrong in two ways — native Python
        ...
        break unquoted bash interpolations. Explicit ``terminal.temp_dir`` /
        ``TMPDIR`` values still win; otherwise use a dedicated cache dir under
        ``HERMES_HOME`` — single-word path, guaranteed to exist, same string
        resolves in both Git Bash and native Python.
        """
        if _IS_WINDOWS:
            # Process TMP/TEMP normally point at the ambient user temp dir,
            # which often contains spaces. Honor deliberate backend overrides
            # plus TERMINAL_TEMP_DIR/TMPDIR, but keep ambient TMP/TEMP from
            # replacing the controlled managed default.
            candidates = (
                self.env.get("TERMINAL_TEMP_DIR"),
                os.environ.get("TERMINAL_TEMP_DIR"),
                self.env.get("TMPDIR"),
            ...
            for candidate in candidates:
                ...
                if candidate_path.is_absolute() and candidate_path.is_dir():
                    ...
                    return normalized if re.fullmatch(r"[A-Za-z]:/", normalized) else normalized.rstrip("/")
