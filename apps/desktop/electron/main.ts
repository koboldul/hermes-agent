...
appOn('hermes:connection:active-route', (event, route) => {
...
appHandle('hermes:window:openBrowser', async (_event, tabId) => {
...
appHandle('hermes:secret-storage:get', async () => ({ on: secretStoragePolicy().on }))
appHandle('hermes:secret-storage:set', async (_event: any, on: any) => applySecretStorageEncryption(on === true))
...
appHandle('hermes:connections:update-managed', async (_event, rawId) => requestManagedSshUpdate(rawId))
...
// the gateway (#79886).
appHandle('hermes:profile:remember', async (_event, name) => ({
